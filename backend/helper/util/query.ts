...
export class Query {
  ...
  public parseMongoDB<T>(queryData: T, validationOptions?: ValidationOptions) {
    ...
    for (const key in queryData) {
        ...
        if (
          ...
          if (isValidateFilter) {
            ...
            fieldValidation?.forEach(typeConstructor => {
              if (!Array.isArray(typeConstructor) && typeConstructor !== undefined && typeConstructor !== null) {
                isValidate = !isValidate ? new Object(value) instanceof typeConstructor : isValidate
          ...
          if (typeof value === 'string' || typeof value === 'number') {
            ...
            listQueryValue = parsedStrValue?.replace(/\s/g, '').split(',').filter(elem => elem)
                  ...
                  if (Array.isArray(listQueryValue)) {
                    listQueryValue?.forEach((queryValue: unknown) => {
                      if (object && new Object(queryValue) instanceof object) {
                        ...
                        isValidate = true
                      } else if (object === undefined && queryValue === undefined) {
      ...
      if (logicalOperator && logicalOperator[value]) {
        runtimeLogical = logicalOperator[value]
      }
      ...
      if (
        ...
        selection = value.replace(/\s/g, '').split(',').reduce((previousValue, currentValue) =>
          currentValue
      ...
      if (!selection) {
        ...
        if (
          nSelectOpr !== undefined
          && typeof value === 'string'
        ) {
          notSelection = value.replace(/\s/g, '').split(',').reduce((previousValue, currentValue) =>
            currentValue
    ...
    const isFilter: boolean = filter?.[defaultLogical]?.length > 0
  ...
  public mergeMongoDB(
    ...
    return query.reduce((previousValue, currentValue) => ({
      ...
      reference: previousValue.reference || currentValue.reference
        ...
        : null
    }), { filter: null, sort: null, projection: null, pagination: null, reference: null })
  }
}
